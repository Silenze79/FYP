-- Migration: rank, rank_points on users + reward_obtain table
-- Safe to run multiple times (IF NOT EXISTS / conditional alters for H2)
...
-- H2: add columns when upgrading an older database
...
-- Backfill rank from points where still default
